...
int FixReaxCBonds::from_tag_to_i(tagint tag){
...
printf("\n=================================\n\n");
  ...
  int a, b,c,d, a_numbonds, b_numbonds, c_numbonds, d_numbonds;
  int i; //the index of an atom in the atom->tag array

  //const
  int const FIRST_TYPE=1; //O TYPE NUM
  int const SECOND_TYPE=2; // H TYPE  NUM
  int const THIRD_TYPE=3;// N TYPE  NUM
  int const FORTH_TYPE=4;// C TYPE  NUM
    ...
    a_type = atom->type[a];
     ...
     =============================*/
      ...
      if(a_type==FIRST_TYPE){
        a_numbonds = nint(numneigh[a]);
        for (b = 0; b < a_numbonds; b++) {
          b_tag = neighid[a][b];
          ...
          if(b_type==SECOND_TYPE){
              b_numbonds=nint(numneigh[i]); //PROBLEM!!!!! need to get b from id
              printf("\nfirst cond OK\n");//**********
              for (c = 0; c < b_numbonds; c++) {
                ...
                c_type = atom->type[i]; //PROBLEM!!!!! need to get type from id
                    ...
                    if(c_type==THIRD_TYPE){
                      printf("\nsecond cond OK\n");//**********
                      c_numbonds=nint(numneigh[i]); //PROBLEM!!!!! need to get c from id
                      for (d = 0; d < c_numbonds; d++) {
                        ...
                        d_type = atom->type[i]; //PROBLEM!!!!! need to get type from id
                          ...
                          if(d_type==FORTH_TYPE){
                            printf("\nthird cond OK\n");//**********
                            d_numbonds=nint(numneigh[i]); //PROBLEM!!!!! need to get d from id
                            for (int e = 0; e < d_numbonds; e++) {
...
void FixReaxCBonds::allocate()
{
  ...
  memory->create(neigh_d,nmax,MAXREAXBOND,"reax/c/bonds:abo");//***************

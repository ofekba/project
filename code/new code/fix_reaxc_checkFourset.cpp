          ...
          if (0.8 <= nbr_p_hn->d && nbr_p_hn->d <= 1.3 ){
            ...
            for( pi3 = start_n; pi3 < end_n; ++pi3 ){
              ...
              if(3.0 <= nbr_p_nc->d && nbr_p_nc->d <= 8.0 ){
                ...
                //if(atom_i4->orig_id==147 || atom_i4->orig_id==146)
               //printf("timestep=%d o_tag=%d, c_tag=%d\n",update->ntimestep, atom_i1->orig_id, atom_i4->orig_id);
  ...
  if(update->ntimestep%100==0){
   /* printf("\n\nat timestep %d\n",update->ntimestep);
    ...
    if(num_fourset!=0){
      ...
      if(apply_flag==1)
        printf("\nstart operate the potential XDXDXDXD\n");
      else
        printf("\ncannnnnnot\n");
    }
  }

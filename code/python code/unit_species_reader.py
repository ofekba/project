...
def species_reader(file_name,time_Step,start_time_Step,No_Moles,No_Specs,trucker,species_at_start,species_at_end):
	i=0
	...
	while i<len(text_list)-1:
		...
		time_Step.append(int(ln[0])+start_time_Step)
		...
		for j in range(len(trucker)):
			if len(trucker[j])<len(time_Step): trucker[j].append(0)
				...
				if i>=len(text_list)-2: 
					species_at_end.append(j)
				if i<=2 and start_time_Step!=0: species_at_start.append(j)
		i+=1
	species_at_end=list(set(species_at_end))
	species_at_end.sort()
...
species_at_end=[]
species_at_start=[]
species_reader("species0.out",time_Step,0,No_Moles,No_Specs,trucker,species_at_start,species_at_end)
species_reader("species1.out",time_Step,2000000,No_Moles,No_Specs,trucker,species_at_start,species_at_end)
species_reader("species2.out",time_Step,3000000,No_Moles,No_Specs,trucker,species_at_start,species_at_end)
species_reader("species3.out",time_Step,4000000,No_Moles,No_Specs,trucker,species_at_start,species_at_end)
...
for _ff in range(len(spec_list)):
	...
	bf="" #for broken fregment
	mf="" #for missing fregment
...
for i in result_list:
	...
	#print(spec_label,new_label[_freg_name_index])
